...
func Get{{.Name}}ByID(ctx context.Context, db DBTX, id {{.PK.QualifiedType .ModelPkgAlias}}, opts ...QueryOption) (*{{.ModelPkgAlias}}.{{.Name}}, error) {
	...
	if cfg.PreloadAssociations {
		return get{{.NamePlural}}ByIDWithRelations(ctx, db, id, cfg)
	}
	{{end}}
...
func FetchAll{{.NamePlural}}(ctx context.Context, db DBTX, opts ...QueryOption) ([]*{{.ModelPkgAlias}}.{{.Name}}, error) {
	...
	if cfg.PreloadAssociations {
		return fetchAll{{.NamePlural}}WithRelations(ctx, db, clause, args)
	}
	{{else}}
...
{{if .Relations}}
{{if .UseJoinStrategy}}
func get{{.NamePlural}}ByIDWithRelations(ctx context.Context, db DBTX, id {{.PK.QualifiedType .ModelPkgAlias}}, cfg QueryOptions) (*{{.ModelPkgAlias}}.{{.Name}}, error) {
	{{if .HasDeletedAt}}
	query := ` + "`" + `
		SELECT 
	...
	if err != nil {
		return nil, fmt.Errorf("get{{.NamePlural}}ByIDWithRelations(%v): %w", id, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		if err := rows.Scan(scanArgs...); err != nil {
			return nil, fmt.Errorf("get{{.NamePlural}}ByIDWithRelations(%v): scanning row: %w", id, err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("get{{.NamePlural}}ByIDWithRelations(%v): iterating rows: %w", id, err)
	}
	if parent == nil {
		return nil, fmt.Errorf("get{{.NamePlural}}ByIDWithRelations(%v): %w", id, sql.ErrNoRows)
	}

	return parent, nil
}

func fetchAll{{.NamePlural}}WithRelations(ctx context.Context, db DBTX, clause string, args []any) ([]*{{.ModelPkgAlias}}.{{.Name}}, error) {
	query := ` + "`" + `
		WITH p AS (
			SELECT {{.AllColumns "p"}}
			FROM {{.Table}} p
	...
	{{range $idx, $rel := .Relations}}
	{{$target := index $.AllKnownModels $rel.TargetModel}}
	// seen_{{$rel.FieldName}} de-duplicates the row fan-out produced by the
	// {{$rel.FieldName}} JOIN. It is keyed by (parent PK, child PK) in a
...
{{else}}
func get{{.NamePlural}}ByIDWithRelations(ctx context.Context, db DBTX, id {{.PK.QualifiedType .ModelPkgAlias}}, cfg QueryOptions) (*{{.ModelPkgAlias}}.{{.Name}}, error) {
	m, err := Get{{.Name}}ByID(ctx, db, id, PreloadAssociations(false))
	if err != nil {
		return nil, err
	}
	...
	{{if eq $rel.Type "HasMany"}}
	{
		children, err := FetchAll{{$target.NamePlural}}(ctx, db, Where("{{$rel.ForeignKey}} = $1", m.{{$.PK.Name}}))
		if err != nil {
	...
	if m.{{$parentFK.Name}} != nil {
		child, err := Get{{$target.Name}}ByID(ctx, db, *m.{{$parentFK.Name}})
	...
	{{if $targetFK}}
	{
		...
		if len(parentIDs) > 0 {
			children, err := batchIn(ctx, db, parentIDs, func(ctx context.Context, db DBTX, batch []any) ([]*{{$.ModelPkgAlias}}.{{$target.Name}}, error) {
				return FetchAll{{$target.NamePlural}}(ctx, db, In("{{$rel.ForeignKey}}", batch...))
			})
			if err != nil {
				return nil, fmt.Errorf("fetchAll{{$.Name}}sWithRelations: preloading {{$rel.FieldName}}: %w", err)
			}
	...
	{{if $parentFK}}
	{
		...
		if len(fkIDs) > 0 {
			children, err := batchIn(ctx, db, fkIDs, func(ctx context.Context, db DBTX, batch []any) ([]*{{$.ModelPkgAlias}}.{{$target.Name}}, error) {
				return FetchAll{{$target.NamePlural}}(ctx, db, In("{{$target.PK.Column}}", batch...))
			})
			if err != nil {
				return nil, fmt.Errorf("fetchAll{{$.Name}}sWithRelations: preloading {{$rel.FieldName}}: %w", err)
			}
