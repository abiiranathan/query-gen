...
func compile() {
	...
	for _, value := range irregularInflections {
		infs := []inflection{
			...
			{regexp: regexp.MustCompile(titleCaser.String(value.singular) + "$"), replace: titleCaser.String(value.plural)},
	...
	for _, value := range irregularInflections {
		infs := []inflection{
			...
			{regexp: regexp.MustCompile(titleCaser.String(value.plural) + "$"), replace: titleCaser.String(value.singular)},
