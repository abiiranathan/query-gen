...
func writeSchemaFile(models []Model, modelMap map[string]Model, dialect SchemaDialect, outPath string) error {
	...
	// First pass: emit every model's CREATE TABLE so join tables (which may
	// reference any model regardless of relation direction) always have
	// both endpoints already defined.
	...
	// Second pass: emit many-to-many join tables now that all parent and
	// target tables exist.
...
// are emitted before dependent tables. Both RelBelongsTo and RelManyToMany
// edges are traversed, since join tables created for RelManyToMany relations
// also require their target table to exist first. Self-referential relations
...
func orderModelsByDependency(models []Model, modelMap map[string]Model) []Model {
	...
	visit = func(m Model, stack map[string]bool) {
		...
		for _, rel := range m.Relations {
			...
			if rel.Type != RelBelongsTo && rel.Type != RelManyToMany {
				continue
			}
